...
///   WillLandFace = initial_IsFace XOR (k is odd).
/// </summary>
public static class StackPeelOffPredictor
{
        ...
        List<CapPrediction> results)
    {
        ...
        if (stack.Count == 1)
        {
            ...
            bool willLandFace = !cap.IsFace;
            results.Add(new CapPrediction(
                cap, depth, startPos, direction, force, travelDistance, willLandFace, source));
        ...
        List<CapPrediction> results)
    {
        ...
        while (workingStack.Count > 0)
        {
            ...
            // Iteration 1 (odd): flipped once → !IsFace.
            // Iteration 2 (even): flipped twice → back to IsFace.
            // Iteration 3 (odd): flipped 3 times → !IsFace.
            // This produces the alternating pattern:
            // 2-stack [h,h] → [t,h], 3-stack [h,h,h] → [t,h,t], etc.
            bool willLandFace = (iteration % 2) == 1 ? !dropCap.IsFace : dropCap.IsFace;
