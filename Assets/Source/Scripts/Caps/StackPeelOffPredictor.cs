...
public static class StackPeelOffPredictor
{
        ...
        Vector2 thrownLandingPos = default)
    {
        ...
        if (thrownCap != null)
        {
            ...
            simIsHeads[thrownCap] = !thrownCap.IsFace;
        ...
        for (int i = 0; i < directHits.Count; i++)
        {
            ...
            ProcessCapLaunch(
                ...
                simPositions, simConsumed, simIsHeads, simPeeledOff, simInStack, simStackAbove,
        ...
        List<CapPrediction> results)
    {
        ...
        if (stack.Count <= 1)
        {
            ...
            bool currentIsHeads = simIsHeads.TryGetValue(cap, out bool h) ? h : cap.IsFace;
        ...
        List<CapPrediction> results)
    {
        ...
        while (workingStack.Count > 0)
        {
            ...
            for (int i = 0; i < workingStack.Count; i++)
            {
                ...
                if (c == null) continue;
                bool curH = simIsHeads.TryGetValue(c, out bool ch) ? ch : c.IsFace;
                simIsHeads[c] = !curH;
            }

            // The dropped cap's WillLandHeads is the toggled value we just set.
            bool willLandHeads = simIsHeads.TryGetValue(dropCap, out bool h) ? h : dropCap.IsFace;
